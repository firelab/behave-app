...
(defprotocol IPersisentSortedSetStorage
...
(defprotocol IBranch
  (toString [_])
  (getAddress [_ idx])
  (setAddress [_ idx new-address]))

(defn binary-search [sorted-vec target cmp]
  (let [search (fn search [low high]
                 (when (<= low high)
                   (let [mid     (int (Math/floor (/ (+ low high) 2)))
                         mid-val (nth sorted-vec mid)]
                     (cond
                       (= (cmp mid-val target) 0) mid
                       (< (cmp mid-val target) 0) (search (inc mid) high)
                       :else                      (search low (dec mid))))))]
    (search 0 (dec (count sorted-vec)))))

(binary-search (range 1 30) 23 compare)

(declare ->Leaf)
(declare ->Branch)

(deftype Node [len keys settings]
  Object
  (toString [this]
    (str "Node: " len keys settings))
  (getLen [_]
    len)
  (minKey [_]
    (first keys))
  ...
  (searchFirst [_ target-key cmp]
    (let [search (fn search [low high]
                   (if-not (< low high)
                     low
                     (let [mid     (unsigned-bit-shift-right (+ high low) 1)
                           mid-val (nth keys mid)
                           d       (cmp mid-val target-key)]
                       (cond
                         (< d 0) (search (+ low mid) high)
                         :else   (search low mid)))))]
      (search 0 len)))
                   ...
                   (if-not (< low high)
                     ...
                     (let [mid     (unsigned-bit-shift-right (+ high low) 1)
                       ...
                       (cond
                         ...
                         :else    (search low mid)))))]
      (search 0 len)))
  (restore [_ level keys addresses settings]
    (if (or (empty? addresses) (= 0 level))
      (->Leaf {:keys keys :settings settings})
      (->Branch {:level level :keys keys :addresses addresses :settings settings} ))))

(defn ->Node [{:keys [len keys addresses settings]}]
  (let [keys (clj->js keys)]
    (assert (>= (count keys) len))
    (Node. len keys settings)))

;; Example Usage:
(let [node (->Node {:len       15
                    :keys      (range 0 30)
                    :addresses ["A1" "A2" "A3"]
                    :settings  {}})]
  (.search node 12 compare)
  (.editable node))
...
(deftype Branch [level len keys ^:volatile-mutable addresses ^:volatile-mutable children settings]
  ...
  (ensureAddresses [_]
    (when (nil? addresses)
      (set! addresses (js/Array. (count keys)))))

  (getAddress [this idx]
    (assert (<= 0 idx (count addresses)))
    (if (nil? addresses)
      nil
      (nth addresses idx)))

  (setAddress [this idx new-address]
    (assert (<= 0 idx (count addresses)))
    (.ensureAddresses this)
    (arrays/aset addresses idx new-address)
    (when (and new-address (= Node (type (nth children idx)))) 
      (arrays/aset children idx (js/WeakRef. new-address)))
    new-address)

  ;;; Children
  (child [this ^IIPersisentSortedSetStorage storage idx]
    (assert (<= 0 idx len))
    (assert (or (and (some? children)
                     (some? (nth children idx)))
                (and (some? addresses)
                     (some? (nth addresses idx)))))

    (let [the-child (let [c (nth children idx)]
                      (when (= js/WeakRef (type c))
                        (.deref c)))]
      
      (if (some? the-child)
        (do
          (when (and (some? addresses)
                     (some? (nth addresses idx)))
            (-accessed storage (nth addresses idx)))
          the-child)
        (do
          (assert (some? (nth addresses idx)))
          (let [the-child (-restore storage (nth addresses idx))]
            (.ensureChild this idx (js/WeakRef the-child))
            the-child)))))

  (ensureChildren [_]
    (when (nil? children)
      (set! children (js/Array. (count keys)))))

  (setChild [this idx child]
    (.address this idx)
    (.ensureChildren this)
    (arrays/aset children idx child)
    child)

  (countChildren [this ^IPersisentSortedSetStorage storage]
    (reduce (fn [sum idx]
              (let [a-child (.child this storage idx)]
                (+ sum (.countChildren a-child))))
            0
            (range 0 (dec len))))
  ...
  (store [this ^ISortedSetStorage storage] 
    ...
    (loop [i 0]
      (when (< i len)
        (when (nil? (nth addresses i))
          (assert (some? children))
          (assert (some? (nth children i)))
          (.store (.address this (nth children i)) storage))
        (recur (inc i))))
    (-store storage this)))

(defn ->Branch [{:keys [level len keys addresses children settings]}]
  (let [keys      (clj->js keys)
        addresses (clj->js addresses)
        children  (clj->js children)]
    (Branch. level len keys addresses children settings)))

(defn walk-addresses
  "Visit each address used by this set. Usable for cleaning up
   garbage left in storage from previous versions of the set"
  [^PersistentSet set consume-fn]
  (.walkAddresses set consume-fn))

(comment 
  ;; Example Usage:
  (let [branch (->Branch {:len       10
                          :level     0
                          :keys      [:k1 :k2 :k3 :k4]
