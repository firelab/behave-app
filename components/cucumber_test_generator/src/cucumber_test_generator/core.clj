...
(defn collect-group-hierarchy
  "Recursively collect all parent groups from a group up to the submodule.
   Returns a vector of {:name :key} tuples in order from root (closest to submodule) to leaf.

   Using {:name :key} tuples instead of bare strings ensures that two sibling groups
   sharing the same translated name (e.g. two groups both named 'Fuel Model') produce
   distinct path vectors, because their :group/translation-key values differ.

   Example: For 'Live Woody Fuel Moisture' nested under 'By Size Class',
            returns [{:name \"By Size Class\" :key \"…:by_size_class\"}
                     {:name \"Live Woody Fuel Moisture\" :key \"…:live_woody\"}]
   ...
   Returns:
   Vector of {:name :key} tuples from root to leaf
  ...
  (let [group    (d/pull db '[:db/id
                         ...
                         group-eid)
        group-el {:name (get-translation db (:group/translation-key group))
                  :key  (:group/translation-key group)}]
    (if-let [parent-group (:group/_children group)]
      ;; Has a parent, recur and prepend current element
      (conj (collect-group-hierarchy db (:db/id parent-group)) group-el)
  ...
  (let [gv (d/pull db
    ...
    (when (seq gv)
      (let [parent-group     (:group/_group-variables gv)
            ...
            ;; Use {:name :key} tuples so siblings with identical display names stay distinct
            module-name      {:name (get-translation db (:module/translation-key parent-module))
                              :key  (:module/translation-key parent-module)}
            submodule-name   {:name (:submodule/name parent-submodule)
                              :key  (:submodule/translation-key parent-submodule)}
...
(defn extract-submodule-info
  ...
  (let [submodule         (pull-submodule-details db submodule-eid)
        ...
        module-name       {:name (get-translation db (:module/translation-key parent-module))
                           :key  (:module/translation-key parent-module)}
        submodule-name    {:name (:submodule/name submodule)
                           :key  (:submodule/translation-key submodule)}
...
(defn generate-edn-data
  ...
  (let [all-groups     (vec (keep #(extract-group-info db %) groups))
        ...
        ;; Combine groups and submodules; drop any whose path contains a nil element
        ;; (signals missing module/submodule translations in the VMS database).
        all-entities   (->> (concat all-groups all-submodules)
                            (remove #(path-has-nil? (:path %))))
        ;; Fail loudly if two entities share the same path key — silent overwrites hide data loss.
        ;; With {:name :key} tuple path elements this should never happen (translation-keys are unique),
        ;; but this guard catches any future regression.
        _              (let [by-path (group-by :path all-entities)
                             dupes   (filter (fn [[_ vs]] (> (count vs) 1)) by-path)]
                         (when (seq dupes)
                           (throw (ex-info "Duplicate path keys in test matrix — two entities resolved to the same path. Check for duplicate translation-keys in the VMS."
                                           {:duplicates (mapv (fn [[path entities]]
                                                                {:path     path
                                                                 :entities (mapv #(select-keys % [:path :group/translated-name :submodule/name]) entities)})
                                                              dupes)}))))
        path-map       (into {} (map (fn [entity] [(:path entity) (dissoc entity :path)]) all-entities))]
    path-map))
