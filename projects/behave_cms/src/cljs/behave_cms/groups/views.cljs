...
(defn manage-group [{submodule-id :db/id}]
  (let [group (rf/subscribe [:state :group])]
    [:div.col-6
     [:h3 (str (if @group "Add" "Update") " Group")
...
(defn list-groups-page [{submodule-id :db/id}]
  (let [submodule (rf/subscribe [:entity submodule-id])
    ...
    [:<>
     [sidebar
      ...
      @groups
      "Submodules"
      (str "/modules/" (:module/rid @submodule))]
     [window sidebar-width
      [:div.container
       [:div.row.mb-3.mt-4
        [:h2 (str (:submodule/name @submodule) " (" (:submodule/io @submodule) ")")]]
       [accordion
        "Groups"
