...
Feature: Surface Input - Fuel Moisture -> By Size Class -> 10-h Fuel Moisture
  ...
  @core
  Scenario Outline: 10-h Fuel Moisture is displayed with these Fuel Model Code
  ...
  @extended
  Scenario Outline: 10-h Fuel Moisture is displayed with these Fuel Model Code (Extended)
    Given I have started a new Surface Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group        | value        |
      | Fire Behavior | Surface Fire | Flame Length |
