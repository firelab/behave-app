@core
Feature: Mortality Results - Crown Length Scorched Backing

  @core
  Scenario Outline: Crown Length Scorched Backing is displayed in results when inputs are set
  ...
  @extended
  Scenario Outline: Crown Length Scorched Backing is displayed in results when inputs are set (Extended)
    Given I have started a new Surface & Mortality Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group          | value                      |
      | Fire Behavior | Direction Mode | Heading, Flanking, Backing |
