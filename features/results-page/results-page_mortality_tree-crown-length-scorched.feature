@core
Feature: Mortality Results - Tree Crown Length Scorched

  @core
  Scenario Outline: Tree Crown Length Scorched is displayed in results when inputs are set
  ...
  @extended
  Scenario Outline: Tree Crown Length Scorched is displayed in results when inputs are set (Extended)
    Given I have started a new Surface & Mortality Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group          | value          |
      | Fire Behavior | Direction Mode | Heading        |
