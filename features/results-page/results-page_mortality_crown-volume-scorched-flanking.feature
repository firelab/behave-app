@core
Feature: Mortality Results - Crown Volume Scorched Flanking

  @core
  Scenario Outline: Crown Volume Scorched Flanking is displayed in results when inputs are set
    ...
    Then "the following outputs are displayed in the results page"
      | output                         |
      | Crown Volume Scorched Flanking |
  ...
  @extended
  Scenario Outline: Crown Volume Scorched Flanking is displayed in results when inputs are set (Extended)
    Given I have started a new Surface & Mortality Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group          | value                      |
      | Fire Behavior | Direction Mode | Heading, Flanking, Backing |
