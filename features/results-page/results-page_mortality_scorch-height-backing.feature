@core
Feature: Mortality Results - Scorch Height Backing

  @core
  Scenario Outline: Scorch Height Backing is displayed in results when inputs are set
  ...
  @extended
  Scenario Outline: Scorch Height Backing is displayed in results when inputs are set (Extended)
    Given I have started a new Surface & Mortality Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group          | value                      |
      | Fire Behavior | Direction Mode | Heading, Flanking, Backing |
