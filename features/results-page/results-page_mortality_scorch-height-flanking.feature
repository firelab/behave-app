@core
Feature: Mortality Results - Scorch Height Flanking

  @core
  Scenario Outline: Scorch Height Flanking is displayed in results when inputs are set
  ...
  @extended
  Scenario Outline: Scorch Height Flanking is displayed in results when inputs are set (Extended)
    Given I have started a new Surface & Mortality Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group          | value                      |
      | Fire Behavior | Direction Mode | Heading, Flanking, Backing |
