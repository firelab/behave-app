@core
Feature: Mortality Results - Crown Length Scorched Flanking

  @core
  Scenario Outline: Crown Length Scorched Flanking is displayed in results when inputs are set
    ...
    Then "the following outputs are displayed in the results page"
      | output                         |
      | Crown Length Scorched Flanking |
  ...
  @extended
  Scenario Outline: Crown Length Scorched Flanking is displayed in results when inputs are set (Extended)
    Given I have started a new Surface & Mortality Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group          | value                      |
      | Fire Behavior | Direction Mode | Heading, Flanking, Backing |
