...
Feature: Surface Input - Fuel Moisture -> Dead, Live Herb, and Live Woody Categories -> Dead Fuel Moisture
  ...
  @core
  Scenario Outline: Dead Fuel Moisture is displayed with these Fuel Model Code
  ...
  @extended
  Scenario Outline: Dead Fuel Moisture is displayed with these Fuel Model Code (Extended)
    Given I have started a new Surface Worksheet in Guided Mode
    When these output paths are selected
      | submodule     | group        | value        |
      | Fire Behavior | Surface Fire | Flame Length |
