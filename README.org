...
#+BEGIN_SRC bash
  ...
  echo 'export PATH="$HOME/.datomic/current/bin:$PATH"' >> ~/.bashrc
#+END_SRC

*** Postgres Setup
#+BEGIN_SRC bash
  cd ~/.datomic/current/bin/sql
  psql -f postgres-user.sql
  psql -f postgres-table.sql
  psql -f postgres-db.sql
#+END_SRC

*** Start Transactor
#+BEGIN_SRC bash
  transactor -Ddatomic.printConnectionInfo=true config/datomic-sql.properties
#+END_SRC
