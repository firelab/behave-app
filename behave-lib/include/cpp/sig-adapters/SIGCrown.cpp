...
SIGCrown::SIGCrown(FuelModels& fuelModels)
...
SIGCrown& SIGCrown::operator=(const SIGCrown& rhs)
{
    if (this != &rhs)
    {
        memberwiseCopyAssignment(rhs);
...
void SIGCrown::memberwiseCopyAssignment(const SIGCrown& rhs)
{
...
void SIGCrown::doCrownRunRothermel()
{
...
void SIGCrown::doCrownRunScottAndReinhardt()
{
...
void SIGCrown::calculateCrownFractionBurned()
{
...
void SIGCrown::assignFinalFireBehaviorBasedOnFireType(CrownModelType::CrownModelTypeEnum crownModelType)
{
...
void SIGCrown::calculateCrownFireActiveWindSpeed()
{
...
double SIGCrown::getCrownFireSpreadRate(SpeedUnits::SpeedUnitsEnum spreadRateUnits) const
{
    return SpeedUnits::fromBaseUnits(crownFireSpreadRate_, spreadRateUnits);
}

double SIGCrown::getSurfaceFireSpreadRate(SpeedUnits::SpeedUnitsEnum spreadRateUnits) const
{
    return surfaceFuel_.getSpreadRate(spreadRateUnits);
}

double SIGCrown::getCrownFirelineIntensity(FirelineIntensityUnits::FirelineIntensityUnitsEnum firelineIntensityUnits) const
{
    return FirelineIntensityUnits::fromBaseUnits(crownFirelineIntensity_, firelineIntensityUnits);
}

double SIGCrown::getCrownFlameLength(LengthUnits::LengthUnitsEnum flameLengthUnits) const
{
    return LengthUnits::fromBaseUnits(crownFlameLength_, flameLengthUnits);
}

FireType::FireTypeEnum SIGCrown::getFireType() const
{
    return fireType_;
}

double SIGCrown::getFinalSpreadRate(SpeedUnits::SpeedUnitsEnum spreadRateUnits) const
{
    return SpeedUnits::fromBaseUnits(finalSpreadRate_, spreadRateUnits);
}

double SIGCrown::getFinalHeatPerUnitArea(HeatPerUnitAreaUnits::HeatPerUnitAreaUnitsEnum heatPerUnitAreaUnits) const
{
    return HeatPerUnitAreaUnits::fromBaseUnits(finalHeatPerUnitArea_, heatPerUnitAreaUnits);
}

double SIGCrown::getFinalFirelineIntesity(FirelineIntensityUnits::FirelineIntensityUnitsEnum firelineIntensityUnits) const
{
    return FirelineIntensityUnits::fromBaseUnits(finalFirelineIntesity_, firelineIntensityUnits);
}

double SIGCrown::getFinalFlameLength(LengthUnits::LengthUnitsEnum flameLengthUnits) const
{
    return LengthUnits::fromBaseUnits(finalFlameLength_, flameLengthUnits);
}

double SIGCrown::getCrownFireLengthToWidthRatio() const
{
    return crownFireLengthToWidthRatio_;
}

double SIGCrown::getCriticalOpenWindSpeed(SpeedUnits::SpeedUnitsEnum speedUnits) const
{
    return SpeedUnits::fromBaseUnits(crownFireActiveWindSpeed_, speedUnits);
}

double SIGCrown::getCrownFractionBurned() const
{
    return crownFractionBurned_;
}

void SIGCrown::initializeMembers()
{
...
void SIGCrown::setFuelModels(FuelModels& fuelModels)
{
    fuelModels_ = &fuelModels;
}

void SIGCrown::calculateCanopyHeatPerUnitArea()
{
...
void SIGCrown::calculateCrownFireHeatPerUnitArea()
{
    crownFireHeatPerUnitArea_ = surfaceFireHeatPerUnitArea_ + canopyHeatPerUnitArea_;
}

void SIGCrown::calculateCrownFuelLoad()
{
...
void SIGCrown::calculateCrownFireTransitionRatio()
{
...
void SIGCrown::calculateCrownFirelineIntensity()
{
    crownFirelineIntensity_ = (crownFireSpreadRate_ / 60.0) * crownFireHeatPerUnitArea_;
}

void SIGCrown::calculateCrownCriticalSurfaceFireIntensity()
{
...
void SIGCrown::calculatePassiveCrownFlameLength()
{
    // Uses Thomas's (1963) flame length (ft) given a fireline intensity (Btu/ft/s).
...
void SIGCrown::calculateSurfaceFireCriticalSpreadRateScottAndReinhardt()
{
    // Scott & Reinhardt's critical surface fire spread rate (R'initiation) (ft/min)
    surfaceFireCriticalSpreadRate_ = (60. * crownCriticalSurfaceFirelineIntensity_) / surfaceFireHeatPerUnitArea_;
}

void SIGCrown::calculateCrownCriticalFireSpreadRate()
{
...
void SIGCrown::calculateCrownFireActiveRatio()
{
...
void SIGCrown::calculateWindSpeedAtTwentyFeet()
{
...
void SIGCrown::calculateCrownLengthToWidthRatio()
{
...
void SIGCrown::calculateCrowningSurfaceFireRateOfSpread()
{
...
void SIGCrown::calculateFireTypeRothermel()
{
    fireType_ = FireType::Surface;
    isActiveCrownFire_ = false;
    isPassiveCrownFire_ = false;
...
void SIGCrown::calculateFireTypeScottAndReinhardt()
{
    // Final fire type
...
void SIGCrown::updateCrownInputs(int fuelModelNumber, double moistureOneHour, double moistureTenHour, double moistureHundredHour,
...
void SIGCrown::setCanopyBaseHeight(double canopyBaseHeight, LengthUnits::LengthUnitsEnum heightUnits)
{
    crownInputs_.setCanopyBaseHeight(canopyBaseHeight, heightUnits);
}

void SIGCrown::setCanopyBulkDensity(double canopyBulkDensity, DensityUnits::DensityUnitsEnum densityUnits)
{
    crownInputs_.setCanopyBulkDensity(canopyBulkDensity, densityUnits);
}

void SIGCrown::setMoistureFoliar(double moistureFoliar, MoistureUnits::MoistureUnitsEnum moistureUnits)
{
    crownInputs_.setMoistureFoliar(moistureFoliar, moistureUnits);
}

std::string SIGCrown::getFuelCode(int fuelModelNumber) const
{
    return fuelModels_->getFuelCode(fuelModelNumber);
}

std::string SIGCrown::getFuelName(int fuelModelNumber) const
{
    return fuelModels_->getFuelName(fuelModelNumber);
}

double SIGCrown::getFuelbedDepth(int fuelModelNumber, LengthUnits::LengthUnitsEnum lengthUnits) const
{
    return fuelModels_->getFuelbedDepth(fuelModelNumber, lengthUnits);
}

double SIGCrown::getFuelMoistureOfExtinctionDead(int fuelModelNumber, MoistureUnits::MoistureUnitsEnum moistureUnits) const
{
    return fuelModels_->getMoistureOfExtinctionDead(fuelModelNumber, moistureUnits);
}

double SIGCrown::getFuelHeatOfCombustionDead(int fuelModelNumber, HeatOfCombustionUnits::HeatOfCombustionUnitsEnum heatOfCombustionUnits) const
{
    return fuelModels_->getHeatOfCombustionDead(fuelModelNumber, heatOfCombustionUnits);
}

double SIGCrown::getFuelHeatOfCombustionLive(int fuelModelNumber, HeatOfCombustionUnits::HeatOfCombustionUnitsEnum heatOfCombustionUnits) const
{
    return fuelModels_->getHeatOfCombustionLive(fuelModelNumber, heatOfCombustionUnits);
}

double SIGCrown::getFuelLoadOneHour(int fuelModelNumber, LoadingUnits::LoadingUnitsEnum loadingUnits) const
{
    return fuelModels_->getFuelLoadOneHour(fuelModelNumber, loadingUnits);
}

double SIGCrown::getFuelLoadTenHour(int fuelModelNumber, LoadingUnits::LoadingUnitsEnum loadingUnits) const
{
    return fuelModels_->getFuelLoadTenHour(fuelModelNumber, loadingUnits);
}

double SIGCrown::getFuelLoadHundredHour(int fuelModelNumber, LoadingUnits::LoadingUnitsEnum loadingUnits) const
{
    return fuelModels_->getFuelLoadHundredHour(fuelModelNumber, loadingUnits);
}

double SIGCrown::getFuelLoadLiveHerbaceous(int fuelModelNumber, LoadingUnits::LoadingUnitsEnum loadingUnits) const
{
    return fuelModels_->getFuelLoadLiveHerbaceous(fuelModelNumber, loadingUnits);
}

double SIGCrown::getFuelLoadLiveWoody(int fuelModelNumber, LoadingUnits::LoadingUnitsEnum loadingUnits) const
{
    return fuelModels_->getFuelLoadLiveWoody(fuelModelNumber, loadingUnits);
}

double SIGCrown::getFuelSavrOneHour(int fuelModelNumber, SurfaceAreaToVolumeUnits::SurfaceAreaToVolumeUnitsEnum savrUnits) const
{
    return fuelModels_->getSavrOneHour(fuelModelNumber, savrUnits);
}

double SIGCrown::getFuelSavrLiveHerbaceous(int fuelModelNumber, SurfaceAreaToVolumeUnits::SurfaceAreaToVolumeUnitsEnum savrUnits) const
{
    return fuelModels_->getSavrLiveHerbaceous(fuelModelNumber, savrUnits);
}

double SIGCrown::getFuelSavrLiveWoody(int fuelModelNumber, SurfaceAreaToVolumeUnits::SurfaceAreaToVolumeUnitsEnum savrUnits) const
{
    return fuelModels_->getSavrLiveWoody(fuelModelNumber, savrUnits);
}

bool SIGCrown::isFuelDynamic(int fuelModelNumber) const
{
    return fuelModels_->getIsDynamic(fuelModelNumber);
}

bool SIGCrown::isFuelModelDefined(int fuelModelNumber) const
{
    return fuelModels_->isFuelModelDefined(fuelModelNumber);
}

bool SIGCrown::isFuelModelReserved(int fuelModelNumber) const
{
    return fuelModels_->isFuelModelReserved(fuelModelNumber);
}

bool SIGCrown::isAllFuelLoadZero(int fuelModelNumber) const
{
    return fuelModels_->isAllFuelLoadZero(fuelModelNumber);
}

void SIGCrown::updateCrownsSurfaceInputs(int fuelModelNumber, double moistureOneHour, double moistureTenHour,
...
void  SIGCrown::setCanopyCover(double canopyCover, CoverUnits::CoverUnitsEnum coverUnits)
{
    surfaceFuel_.setCanopyCover(canopyCover, coverUnits);
}

void  SIGCrown::setCanopyHeight(double canopyHeight, LengthUnits::LengthUnitsEnum canopyHeightUnits)
{
    surfaceFuel_.setCanopyHeight(canopyHeight, canopyHeightUnits);
}

void  SIGCrown::setCrownRatio(double crownRatio)
{
    surfaceFuel_.setCrownRatio(crownRatio);
}

void  SIGCrown::setFuelModelNumber(int fuelModelNumber)
{
    surfaceFuel_.setFuelModelNumber(fuelModelNumber);
}

void  SIGCrown::setMoistureOneHour(double moistureOneHour, MoistureUnits::MoistureUnitsEnum moistureUnits)
{
    surfaceFuel_.setMoistureOneHour(moistureOneHour, moistureUnits);
}

void  SIGCrown::setMoistureTenHour(double moistureTenHour, MoistureUnits::MoistureUnitsEnum moistureUnits)
{
    surfaceFuel_.setMoistureTenHour(moistureTenHour, moistureUnits);
}

void  SIGCrown::setMoistureHundredHour(double moistureHundredHour, MoistureUnits::MoistureUnitsEnum moistureUnits)
{
    surfaceFuel_.setMoistureHundredHour(moistureHundredHour, moistureUnits);
}

void  SIGCrown::setMoistureLiveHerbaceous(double moistureLiveHerbaceous, MoistureUnits::MoistureUnitsEnum moistureUnits)
{
    surfaceFuel_.setMoistureLiveHerbaceous(moistureLiveHerbaceous, moistureUnits);
}

void  SIGCrown::setMoistureLiveWoody(double moistureLiveWoody, MoistureUnits::MoistureUnitsEnum moistureUnits)
{
    surfaceFuel_.setMoistureLiveWoody(moistureLiveWoody, moistureUnits);
}

void  SIGCrown::setSlope(double slope, SlopeUnits::SlopeUnitsEnum slopeUnits)
{
    surfaceFuel_.setSlope(slope, slopeUnits);
}

void  SIGCrown::setAspect(double aspect)
{
    surfaceFuel_.setAspect(aspect);
}

void  SIGCrown::setWindSpeed(double windSpeed, SpeedUnits::SpeedUnitsEnum windSpeedUnits, WindHeightInputMode::WindHeightInputModeEnum windHeightInputMode)
{
    surfaceFuel_.setWindSpeed(windSpeed, windSpeedUnits, windHeightInputMode);
}

void  SIGCrown::setWindDirection(double windDirection)
{
...
void  SIGCrown::setWindAndSpreadOrientationMode(WindAndSpreadOrientationMode::WindAndSpreadOrientationModeEnum windAndSpreadAngleMode)
{
    surfaceFuel_.setWindAndSpreadOrientationMode(windAndSpreadAngleMode);
}

void SIGCrown::setUserProvidedWindAdjustmentFactor(double userProvidedWindAdjustmentFactor)
{
    surfaceFuel_.setUserProvidedWindAdjustmentFactor(userProvidedWindAdjustmentFactor);
}

void SIGCrown::setWindAdjustmentFactorCalculationMethod(WindAdjustmentFactorCalculationMethod::WindAdjustmentFactorCalculationMethodEnum windAdjustmentFactorCalculationMethod)
{
    surfaceFuel_.setWindAdjustmentFactorCalculationMethod(windAdjustmentFactorCalculationMethod);
}

int SIGCrown::getFuelModelNumber() const
{
    return surfaceFuel_.getFuelModelNumber();
}

double SIGCrown::getMoistureOneHour(MoistureUnits::MoistureUnitsEnum moistureUnits) const
{
    return surfaceFuel_.getMoistureOneHour(moistureUnits);
}

double SIGCrown::getMoistureTenHour(MoistureUnits::MoistureUnitsEnum moistureUnits) const
{
    return surfaceFuel_.getMoistureTenHour(moistureUnits);
}

double SIGCrown::getMoistureHundredHour(MoistureUnits::MoistureUnitsEnum moistureUnits) const
{
    return surfaceFuel_.getMoistureHundredHour(moistureUnits);
}

double SIGCrown::getMoistureLiveHerbaceous(MoistureUnits::MoistureUnitsEnum moistureUnits) const
{
    return surfaceFuel_.getMoistureLiveHerbaceous(moistureUnits);
}

double SIGCrown::getMoistureLiveWoody(MoistureUnits::MoistureUnitsEnum moistureUnits) const
{
    return surfaceFuel_.getMoistureLiveWoody(moistureUnits);
}

double SIGCrown::getWindSpeed(SpeedUnits::SpeedUnitsEnum windSpeedUnits, WindHeightInputMode::WindHeightInputModeEnum windHeightInputMode) const
{
    return surfaceFuel_.getWindSpeed(windSpeedUnits, windHeightInputMode);
}

double SIGCrown::getWindDirection() const
{
    return surfaceFuel_.getWindDirection();
}

double SIGCrown::getSlope(SlopeUnits::SlopeUnitsEnum slopeUnits) const
{
    return surfaceFuel_.getSlope(slopeUnits);
}

double SIGCrown::getAspect() const
{
    return surfaceFuel_.getAspect();
}

double SIGCrown::getCanopyCover(CoverUnits::CoverUnitsEnum canopyCoverUnits) const
{
    return surfaceFuel_.getCanopyCover(canopyCoverUnits);
}

double SIGCrown::getCanopyHeight(LengthUnits::LengthUnitsEnum canopyHeighUnits) const
{
    return surfaceFuel_.getCanopyHeight(canopyHeighUnits);
}

double SIGCrown::getCrownRatio() const
{
    return surfaceFuel_.getCrownRatio();
}

double SIGCrown::getCanopyBaseHeight(LengthUnits::LengthUnitsEnum canopyHeightUnits) const
{
    return crownInputs_.getCanopyBaseHeight(canopyHeightUnits);
}

double SIGCrown::getCanopyBulkDensity(DensityUnits::DensityUnitsEnum canopyBulkDensityUnits) const
{
    return crownInputs_.getCanopyBulkDensity(canopyBulkDensityUnits);
}

double SIGCrown::getMoistureFoliar(MoistureUnits::MoistureUnitsEnum moistureUnits) const
{
