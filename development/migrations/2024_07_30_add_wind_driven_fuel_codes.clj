...
(do 
  ...
  (def wind-driven-fuel-code-subgroup-tx
    (merge (sm/->subgroup fuel-models-parent-group
  ...
  ;; Disable Fuel Models when Spot Wind Driven 

  (def standard-fuel-model-group (sm/t-key->eid db "behaveplus:surface:input:fuel_models:standard:fuel_model"))

  (def disable-standard-fuel-model-group-tx
    {:db/id standard-fuel-model-group
     :group/conditionals
     [(sm/->gv-conditional spot-wind-driven-surface-fire-output :equal "false")]})
  ...
  (comment
    (def tx (d/transact conn [wind-driven-fuel-code-list-tx
                              ...
                              disable-standard-fuel-model-group-tx]))
    )
